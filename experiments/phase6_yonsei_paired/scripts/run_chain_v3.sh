#!/usr/bin/env bash
# v3: Wait for 16b to finish → AffectNet finetune (Q1 decision) → 16d product → xeval + aggregate
# (16c dropped — degenerate config fails model assertion; Stage 13 covers that ablation cell)
set -uo pipefail
...
echo "[$(date)] waiting for stage16b_rel_only..."
while pgrep -f "stage16b_rel_only.yaml" > /dev/null; do
  sleep 60
done
echo "[$(date)] stage16b_rel_only DONE"
...
for STAGE in stage16a_iso_only stage16b_rel_only stage16d_product; do
