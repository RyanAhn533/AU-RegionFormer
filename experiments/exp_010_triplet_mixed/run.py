...
OUT = Path("/home/ajy/AU-RegionFormer/experiments/exp_010_triplet_mixed")
...
# MIXED subset: is_selected 모두 (rejected 10% 포함)
mixed_valid = np.where(valid)[0]
print(f"[info] mixed subset — rejected fraction: {meta['is_selected'].values[mixed_valid].mean():.3f}")
...
sel = []
for c in range(4):
    ci = mixed_valid[y_all[mixed_valid] == c]
