...
# Sad class has re-numbered filenames (sad_XXXXX.jpg). Load mapping once.
_SAD_MAPPING = None


def _load_sad_mapping():
    """Build orig_fname → sad_fname dict. Load lazily once."""
    global _SAD_MAPPING
    if _SAD_MAPPING is not None:
        return _SAD_MAPPING
    mapping_path = Path("/home/ajy/AU-RegionFormer/data/label_quality/sad_to_orig_mapping.csv")
    if not mapping_path.exists():
        _SAD_MAPPING = {}
        return _SAD_MAPPING
    df = pd.read_csv(mapping_path)
    _SAD_MAPPING = dict(zip(df["orig_fname"], df["sad_fname"]))
    print(f"[info] loaded sad mapping: {len(_SAD_MAPPING)} entries")
    return _SAD_MAPPING
...
def find_image_path(path_in_meta, emotion_ko, image_roots):
    ...
    슬픔(sad)은 파일명이 re-numbered되어 있어 sad_to_orig_mapping.csv로 변환.
    """
    p = Path(path_in_meta)
    if p.exists():
        return str(p)
    ...
    # Sad 특수 케이스: mapping으로 sad_fname 조회
    if emotion_ko == "슬픔":
        mapping = _load_sad_mapping()
        sad_fname = mapping.get(basename)
        if sad_fname:
            for root in image_roots:
                cand = Path(root) / "sad" / sad_fname
                if cand.exists():
                    return str(cand)
        # Fallback: 원본 basename으로 시도 (매핑 누락분)
        for root in image_roots:
            cand = Path(root) / "sad" / basename
            if cand.exists():
                return str(cand)
        return None
